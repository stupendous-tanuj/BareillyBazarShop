...
throw new Exception($userId_MISSING);
		...
		$query = "SELECT CustomerMobileNumber,CustomerName, ProductDescription, ProductRequest.CreationTime FROM `ProductRequest`,Customer WHERE (ProductRequest.CustomerMobileNumber = Customer.CustomerMobileNumber)";
	...
	while($row=mysqli_fetch_array($result)){
			...
			$dataRow['productDescription']=$row['ProductDescription'];
			$dataRow['creationTime']=$row['CreationTime'];
			$dataResult['response']['productRequest'][$i++]=$dataRow;
		}
	}
	...
	$dataRow = array();
	$dataResult['status']=1;
	$dataResult['response']['productRequest']=$dataRow;
}
else{
...
//catch exception
	...
	$data = array();
	$data['status']=0;
	$data['response']['errorCode']=$FETCH_PRODUCT_REQUESTS_SERVICE_ERROR_CODE;
	$data['response']['errorMessage']=$e->getMessage();
	$data['response']['supportContactNumber']=fetchSupportContactNumber($link,$applicationId);
	logAnError($link,'Fetch_Product_Requests', $_POST, $userId, json_encode($data), $e);
